...
class Multimodal_Attention(nn.Module):
    ...
    def forward(self, x1, mask1, x2, mask2):
        B, N1, C = x1.shape
        B, N2, C = x2.shape
        qkv1 = self.qkv1(x1).reshape(B, N1, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q1, k1, v1 = qkv1[0], qkv1[1], qkv1[2]   # make torchscript happy (cannot use tensor as tuple)
        qkv2 = self.qkv2(x2).reshape(B, N2, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q2, k2, v2 = qkv2[0], qkv2[1], qkv2[2]   # make torchscript happy (cannot use tensor as tuple)

        attn = (q1 @ k2.transpose(-2, -1)) * self.scale
        if mask2 is not None:
            attn = attn.masked_fill(mask2.unsqueeze(1).unsqueeze(1) == 0, -1e9) # mask1的reshape有待确定？

        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v2).transpose(1, 2).reshape(B, N1, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x, mask):
        attn = self.attn(self.norm1(x), mask)
        x = x + self.drop_path(attn)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x

class Multimodal_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn1_2 = Multimodal_Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        self.attn2_1 = Multimodal_Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
    ...
    def forward(self, x1, mask1, x2, mask2):
        attn1 = self.attn1(self.norm1(x1), mask1)
        attn2 = self.attn2(self.norm2(x2), mask2)
        attn1 = self.attn1_2(self.norm1_1(attn1), mask1, self.norm1_2(attn2), mask2)
        attn2 = self.attn2_1(self.norm2_1(attn2), mask2, self.norm2_2(attn1), mask1)
        x1 = x1 + self.drop_path1(attn1)
        x1 = x1 + self.drop_path1(self.mlp1(self.norm2_1(x1)))
        x2 = x2 + self.drop_path2(attn2)
        x2 = x2 + self.drop_path2(self.mlp2(self.norm2_2(x2)))
        return x1, x2
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=10, embed_dim=768, depth=6,
                 num_heads=4, mlp_ratio=1., qkv_bias=True, representation_size=None, distilled=False,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None, weight_init='', cls_token_enable=True, pool='cls'):
        """
        Args:
        ...
        self.pool = pool
        self.cls_token_enable = cls_token_enable
    ...
    def forward_features(self, x, pos_embed, mask):
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        if self.dist_token is None:
            if self.cls_token_enable == True:
    ...
    def forward(self, x, pos_embed, mask):
        x = self.forward_features(x, pos_embed, mask)
...
class MultimodalTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=10, embed_dim=768, depth=6,
        ...
        self.dist_token1 = nn.Parameter(torch.zeros(1, 1, embed_dim)) if distilled else None
        self.dist_token2 = nn.Parameter(torch.zeros(1, 1, embed_dim)) if distilled else None
    ...
    def init_weights(self, mode=''):
        assert mode in ('jax', 'jax_nlhb', 'nlhb', '')
        head_bias = -math.log(self.num_classes) if 'nlhb' in mode else 0.
        if self.dist_token1 is not None:
            trunc_normal_(self.dist_token1, std=.02)
        if self.dist_token2 is not None:
    ...
    def forward_features(self, x1, pos_embed1, mask1, x2, pos_embed2, mask2):
        cls_token1 = self.cls_token1.expand(x1.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        cls_token2 = self.cls_token2.expand(x2.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        if self.dist_token1 is None:
            if self.cls_token_enable == True:
                x1 = torch.cat((cls_token1, x1), dim=1)
            else:
                pass

        if self.dist_token2 is None:
            if self.cls_token_enable == True:
                x2 = torch.cat((cls_token2, x2), dim=1)
            else:
                pass
        else:
            x1 = torch.cat((cls_token1, self.dist_token1.expand(x1.shape[0], -1, -1), x1), dim=1)
            x2 = torch.cat((cls_token2, self.dist_token2.expand(x2.shape[0], -1, -1), x2), dim=1)
        x1 = self.pos_drop1(x1 + pos_embed1)
        x2 = self.pos_drop2(x2 + pos_embed2)
        for enc_layer in self.blocks:
            x1, x2 = enc_layer(x1, mask1, x2, mask2)

        x1 = self.norm1(x1)
        x2 = self.norm2(x2)
        if (self.dist_token1 and self.dist_token2) is None:
            if self.pool == 'cls':
                return self.pre_logits(x1[:, 0]), self.pre_logits(x2[:, 0])
            elif self.pool == 'mean':
                return self.pre_logits(x1.mean(dim = 1)), self.pre_logits(x2.mean(dim = 1))
        else:
            return x1[:, 0], x1[:, 1], x2[:, 0], x2[:, 1]

    def forward(self, x1, pos_embed1, mask1, x2, pos_embed2, mask2):
        x1, x2 = self.forward_features(x1, pos_embed1, mask1, x2, pos_embed2, mask2)
        return x1, x2
